...
int main()
{
    ...
    omp_set_num_threads(11);      // Seteamos el numero de nucleos a usar
    #pragma omp parallel for     // Paralelizamos
    for (int iD=0; iD<nD; iD++)
    {
        for (int ir=0; ir<nr; ir++)
        {
            std::string iSimDm = std::string("iSim_") 
                               + (iD < 10 ? "00" : (iD < 100 ? "0" : "")) + std::to_string(iD)
                               + "_"
                               + (ir < 10 ? "00" : (ir < 100 ? "0" : "")) + std::to_string(ir)
                               + ".txt";   

            if (fs::exists(iSimDm)) continue;

            /* INITIALIZATION */

            /* Parameters */
            float foodRotationRate = 0.00;                          // Fuerza de chemotaxis, Parametro de control
            float angularDiffusion = pow(10, 0.0 - float(iD)*0.2); // Coeficiente de difusion angular
            float activeVelocity   = pow(10, float(iD)*0.2 - 3.0); // Velocidad activa
            ...
            float intakeCapacity = 0.0100 * pow(10, float(ir)*0.2 - 2.0);
            ...
            float nStarvation   = growthRate/intakeSlope * (1.0 - (metabolicRate*starvationEnergy   + kineticRate*activeVelocity*activeVelocity)/(intakeSlope*patchCapacity));
            float nReproduction = growthRate/intakeSlope * (1.0 - (metabolicRate*reproductiveEnergy + kineticRate*activeVelocity*activeVelocity)/(intakeSlope*patchCapacity));
